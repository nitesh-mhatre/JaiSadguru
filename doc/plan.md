...
| W-02 | **Chart interval selection + mobile UI**: interval picker (1m/5m/15m/30m/1h/1d/1wk) on the dashboard; `interval` accepted on `GET /api/market[/symbol]` and `POST /api/forecast/{symbol}`; intraday cache TTLs and per-interval yfinance periods; session-aware intraday forecast timestamps; interval-aware chart time axis (clock times for intraday bars); mobile layout (stacked header, scrollable/condensed tables, tap-sized controls, compact chart) | ✅ Done | Branch `feat/interval-mobile-ui`. **Verified: `python -m compileall backend/app` exit 0; `npm run typecheck` + `npm run build` exit 0.** Live intraday downloads and forecasts not executed here (B-01): run the commands in the Verify section with `?interval=5m` / an interval body to confirm on your machine |
...
| B-06 | Forecast overlay showed the **same candles at every interval** (1m–30m identical) after interval selection landed | `GET /api/forecast/latest` returned one forecast per **symbol** and the signal panel matched only on symbol, so whichever forecast was stored last (always the daily-bar one from Run cycle) was drawn over the intraday history regardless of the chart's bar size | Forecasts are stored with an `interval` column already; `/api/forecast/latest` now accepts `?interval=` and the store gains `latest_forecast_per_symbol_and_interval()` (group by symbol+interval); the dashboard filters both the overlay and the signal by the selected bar size. Re-forecast still required per interval — switching bars shows the *stored* forecast for that bar size, not an on-demand one | ✅ Fixed |
| B-07 | Every intraday forecast (1m–1h) failed with `BTC-USD: lookback 400 exceeds the 240-bar context allowed for intraday interval '1m'` | The W-02 intraday guard **raised** on `lookback > 240`, but requests omit `lookback`, so it resolved to the daily default (400) and every intraday forecast was refused — the guard protected a configuration instead of the user | The forecast service now **clamps** the context to `INTRADAY_MAX_LOOKBACK` (240) for intraday intervals and records a visible note ("context clamped from 400 to 240 bars…") that travels into the forecast and signal rationale (rule R6); the boot-time `Settings` check for `INTERVAL`×`LOOKBACK` was removed for the same reason — the configured daily default must not make the API unbootable when the user selects an intraday interval | ✅ Fixed |
...
| 2026-09-15 | **Interval selection on the chart** (1m/5m/15m/30m/1h/1d/1wk) with the forecast following the selected bar size | User request ("user can select time for chart; if 1 min selected prediction done on 1 min data"). The interval is a per-request parameter, not a server restart: `GET /api/market?interval=…` and the forecast body both accept it. 1m/5m/15m/30m added alongside the existing 1d/1h/1wk; **3m is deliberately absent** because the data provider does not offer it. Intraday stays best-effort per the objective's non-goals — the picker exists, but daily remains the default and intraday contexts are capped at 240 bars |
| 2026-09-15 | **Intraday forecast timestamps replay the last session's times-of-day** across following business days instead of stepping a fixed delta | Stepping 5 minutes through 20:00 Friday lands forecast bars at 03:00 Saturday — a timestamp no 5m bar of an index will ever have, which would misalign scoring (the intraday cousin of B-03). Crypto keeps continuous stepping because it has no sessions |
...
- **2026-09-15** — Fixed B-07 (intraday forecasts refused with "lookback 400 exceeds the 240-bar context"): forecast service clamps intraday context to 240 bars with a visible note instead of raising; removed the boot-time INTERVAL×LOOKBACK validation that could make the API unbootable on a valid per-request combination — `feat/interval-mobile-ui` — **verified: `python -m compileall backend/app` exit 0. Live forecast still unverified here (B-01)**.
- **2026-09-15** — Fixed B-06 (same candles at every interval): `/api/forecast/latest?interval=` filter + `latest_forecast_per_symbol_and_interval()` store query; dashboard overlays the stored forecast and signal matching the selected bar size — `feat/interval-mobile-ui` — **verified: `python -m compileall` pass; `npm run typecheck` exit 0; `npm run build` exit 0 (256.15 kB / 79.12 kB gzip)**.
- **2026-09-15** — Chart interval selection + mobile UI (W-02): `IntervalPicker` segmented control (1m/5m/15m/30m/1h/1d/1wk) above the forecast chart; interval threaded through `/api/market[/symbol]`, `POST /api/forecast/{symbol}` and the forecast service (5m selected → 5-minute bars predicted); per-interval yfinance periods (7d @1m, 60d @5m–1h) and cache TTLs (1–30 min intraday); session-aware intraday `future_timestamps` (replays the last session's times-of-day over business days; crypto stays continuous); chart time axis switches dates ↔ clock times by interval with spaced ticks; interval shown in the chart meta ("10 × 5m") and the signal panel; mobile: stacked header with 2-col button grid, horizontally scrollable watchlist/trade/position tables with low-value columns hidden below 760px/420px, 44px tap targets, 16px search input (no iOS zoom), compact chart — `feat/interval-mobile-ui` — **verified: `python -m compileall backend/app` pass; `npm run typecheck` exit 0; `npm run build` exit 0. Live intraday downloads/forecasts unverified in this sandbox (B-01)**.
