"""Vendored Kronos model implementation.

Source: https://github.com/shiyu-coder/Kronos (MIT License, see ./LICENSE).
Paper:  Kronos: A Foundation Model for the Language of Financial Markets
        (AAAI 2026, https://arxiv.org/abs/2508.02739)

VENDORING CHANGES ONLY (upstream logic is untouched):
  * ``from model.module import *`` -> ``from .kronos_modules import *`` so the code
    resolves as a package instead of requiring the Kronos repo layout.
  * Removed ``sys.path.append("../")``, which only made sense when running the
    upstream example scripts from inside its own checkout.

Do not edit the model maths in this file. If upstream fixes are needed, re-vendor
from the source repo and re-apply the two changes above.
"""
import numpy as np
import pandas as pd
import torch
